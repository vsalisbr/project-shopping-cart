...
export const removeCartID = (id) => {
  ...
  console.log(cartProducts[indexProduct]);
...
export function addAmountToCart(amount) {
  totalPrice.innerText = Number(totalPrice.innerText) + Number(amount);
}

export function subAmountToCart(amount) {
  totalPrice.innerText = Number(totalPrice.innerText) - Number(amount);
}
